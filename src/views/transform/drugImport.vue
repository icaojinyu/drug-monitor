  ...
  <div>
    ...
    <table class="table">
      ...
      <tr v-for="(item,index) in tableData">
        ...
        <td>{{item.drugunit}}</td>
        <td>{{item.drugBuy.buyprice}}</td>
        <td>{{item.drugBuy.onebuyprice}}</td>
        <td>{{item.drugBuy.buynum}}</td>
        <td>{{item.drugBuy.onebuynum}}</td>
        <td>{{item.drugBuy.writedate}}</td>
        <td></td>
      </tr>
    </table>
    <el-pagination
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        breadcrumbData: ['转换数据', '药品监测表', '药品进货监测表'],
        currentPage: 1,
        pageSize: 40,
        tableData: [],
        total: 0,
    ...
    methods: {
      getTableData(page) {
        this.loading = true
        $.get(api.baseUrl + api.getConversionOfDataDrugDelivery + `${page}/${this.pageSize}`).then((res) => {
          console.log(res)
          this.loading = false
      ...
      getSearchData(page) {
        this.loading = true
        $.get(api.baseUrl + api.getConversionOfDataDrugDelivery + `${page}/${this.pageSize}?ypid=${this.ypid}&factoryName=${this.factoryName}&cadnType=${this.cadnType}&drugCode=${this.drugCode}&drugName=${this.drugName}`).then((res) => {
          console.log(res)
          this.loading = false
          this.tableData = res.data
          this.total = res.totalNum
