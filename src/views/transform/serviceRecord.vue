...
<script>
  ...
  export default {
    ...
    data() {
      return {
        breadcrumbData: ['转换数据', '医疗服务监测表', '医疗服务记录表'],
        tableData: [],
        currentPage: 1,
        pageSize: 40,
        total: 0,
    ...
    methods: {
      getTableData(page) {
        this.loading = true
        $.get(api.baseUrl + api.getServiceinfoSell + `${page}/${this.pageSize}`).then((res) => {
          console.log(res)
          this.loading = false
      ...
      getSearchData(page) {
        $.get(api.baseUrl + api.getServiceinfoSell + `${page}/${this.pageSize}?baseCode=${this.baseCode}&baseName=${this.baseName}&serviceCode=${this.serviceCode}`).then((res) => {
          console.log(res)
          this.loading = false
          this.tableData = res.data
          this.total = res.totalNum
