  ...
  <div>
    ...
    <table class="table">
      ...
      <tr v-for="(item,index) in tableData" :key="index">
        ...
        <td>{{item.buyprice}}</td>
        <td>{{item.productminpunitprice}}</td>
        <td>{{item.buynum}}</td>
        <td>{{item.productmiinpunitbuynum}}</td>
      </tr>
    </table>
    <el-pagination
      class="pagination"
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        breadcrumbData: ['转换数据', '高值医用耗材监测表', '耗材进货监测表'],
        tableData: [],
        currentPage: 1,
        pageSize: 40,
        total: 0,
    ...
    methods: {
      getTableData(page) {
        this.loading = true
        $.get(api.baseUrl + api.gethcbuy + `${page}/${this.pageSize}`).then((res) => {
          console.log(res)
          this.loading = false
      ...
      getSearchData(page) {
        $.get(api.baseUrl + api.gethcbuy + `${page}/${this.pageSize}?YNHCBS=${this.YNHCBS}&MATERIALNAME=${this.MATERIALNAME}`).then((res) => {
          console.log(res)
          this.loading = false
          this.tableData = res.data
          this.total = res.totalNum
