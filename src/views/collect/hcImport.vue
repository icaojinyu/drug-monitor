...
<script>
  ...
  export default {
    ...
    data() {
      return {
        breadcrumbData: ['采集数据', '耗材监测表', '高值医用耗材进货监测表'],
        currentPage: 1,
        pageSize: 40,
        tableData: [],
        total: 0,
    ...
    methods: {
      getTableData(page) {
        this.loading = true
        $.get(api.baseUrl + api.getvwhcimport + `${page}/${this.pageSize}`).then((res) => {
          console.log(res)
          this.loading = false
      ...
      getSearchData(page) {
        this.loading = true
        $.get(api.baseUrl + api.getvwhcimport + `${page}/${this.pageSize}?YNHCBS=${this.YNHCBS}&MATERIALNAME=${this.MATERIALNAME}`).then((res) => {
          console.log(res)
          this.loading = false
