...
<script>
  ...
  export default {
    ...
    data() {
      return {
        breadcrumbData: ['标准数据', '服务监测表', '2001版'],
        currentPage: 1,
        pageSize: 40,
        tableData: [],
        total: 0,
    ...
    methods: {
      getTableData(page) {
        this.loading = true
        $.get(api.baseUrl + api.getStandradServices + `/2001/${page}/${this.pageSize}`).then((res) => {
          console.log(res)
          this.loading = false
      ...
      search() {
        if (!this.serviceCode.trim() || !this.projectName.trim()) {
          this.$message({
            message: '请输入搜索内容',
            type: 'warning',
            center: true
      ...
      getSearchData(page) {
        this.loading = true
        $.get(api.baseUrl + api.getStandradServices + `/2001/${page}/${this.pageSize}?servicecode=${this.serviceCode}&projectname=${this.projectName}&projectcontent=${this.projectContent}`).then((res) => {
          console.log(res)
          this.loading = false
          this.tableData = res.data
          this.total = res.totalNum
